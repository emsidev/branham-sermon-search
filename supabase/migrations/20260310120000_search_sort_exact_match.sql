...
chunk_hits AS (
  SELECT
    ...
    (
      lower(trim(sc.chunk_text)) = pp.query_lower
      OR lower(trim(sp.paragraph_text)) = pp.query_lower
    ) AS is_exact_match,
    CASE
...
metadata_hits AS (
  SELECT
    ...
    lower(trim(mc.field_value)) = lower(trim(mc.query_text)) AS is_exact_match,
