...
CREATE INDEX IF NOT EXISTS idx_sermon_pages_page_text_trgm
  ON public.sermon_pages USING GIN (page_text gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_sermons_title_trgm
  ON public.sermons USING GIN (title gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_sermons_scripture_trgm
  ON public.sermons USING GIN (scripture gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_sermons_location_trgm
    ...
    CASE
      WHEN sp.page_fts @@ p.ts_query THEN
        ts_headline(
          ...
          sp.page_text,
          p.ts_query,
          'MaxWords=24,MinWords=10,ShortWord=2,MaxFragments=1,FragmentDelimiter= ... ,StartSel=,StopSel='
        )
