...
import { createHitNavigationKeyHandler, createHitNavigator } from '@/lib/hitNavigation';

export interface UseHitNavigationOptions {
...
export interface UseHitNavigationResult {
  activeIndex: number;
  totalHits: number;
  goNext: () => number;
  goPrev: () => number;
...
export function useHitNavigation({
  containerRef,
  enabled,
  initialIndex = -1,
  ...
  onNextSermon,
  onPrevSermon,
}: UseHitNavigationOptions): UseHitNavigationResult {
