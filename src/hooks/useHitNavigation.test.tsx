...
import { useHitNavigation } from './useHitNavigation';
...
describe('useHitNavigation', () => {
  ...
  it('discovers hit marks and applies initial index', async () => {
    ...
    const { result } = renderHook(() => useHitNavigation({
      containerRef,
      enabled: true,
      initialIndex: 1,
      scrollBehavior: 'auto',
  ...
  it('honors enabled state and resets when hit list changes', async () => {
    ...
    const { result, rerender } = renderHook(
      ({ enabled, initialIndex }: { enabled: boolean; initialIndex: number }) => useHitNavigation({
        containerRef,
        enabled,
        initialIndex,
        scrollBehavior: 'auto',
